...
export default function GorevOlusturClient() {
  ...
  const [secilenKullanicilar, setSecilenKullanicilar] = useState<string[]>([
    "istanbul-il-sorumlusu",
  ]);
  ...
  return (
    <div className="max-w-[920px]">
      <Link
        ...
        className="text-[13px] text-[#243B64] underline"
      >
        Görevlere dön
      </Link>
      <div className="mt-3">
        <PageTitle
          title="Görev oluştur"
          sub="Görev, seçilen illerin sorumlularına atanır."
        />
      </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Field label="Öncelik" htmlFor="f-oncelik">
              <select
                ...
                className={CONTROL}
              >
                {["Kritik", "Yüksek", "Orta", "Düşük"].map((o) => (
      ...
      <Card className="mb-[18px]">
        <CardHead
          title="Görev alıcıları"
          hint={`${secilenKullanicilar.length} kullanıcı seçildi`}
        />
        <div className="grid gap-2.5 p-[18px] sm:grid-cols-2">
          {ATANABILIR_KULLANICILAR.map((user) => {
            const secili = secilenKullanicilar.includes(user.id);
            ...
            return (
              <label
                ...
                className={
                  secili
                    ? "flex cursor-pointer items-start gap-2.5 rounded-lg border border-[#14213D] bg-[#F6F9FE] px-3.5 py-3"
                    : "flex cursor-pointer items-start gap-2.5 rounded-lg border border-[#DDE5F0] px-3.5 py-3"
                }
              >
                <input
                  checked={secili}
                  className="mt-0.5 h-4 w-4 accent-[#14213D]"
                  onChange={() =>
                    setSecilenKullanicilar((current) =>
        ...
        <div className="border-t border-[#DDE5F0] px-[18px] py-3 text-xs text-[#667085]">
          Öğrenci hesapları görev alıcısı olarak listelenmez. Seçilen çalışanlar
      ...
      <Card className="mb-[18px]">
        <CardHead
          title="Atanacak iller"
          hint={`${secilenIller.length} il seçildi`}
        />
      ...
      <div className="flex flex-wrap gap-2.5">
        ...
        {!gecerli ? (
          <span className="self-center text-[13px] text-[#8A5F0F]">
            Başlık, en az bir il ve en az bir görev alıcısı gerekli.
          </span>
        ) : null}
      </div>
    </div>
