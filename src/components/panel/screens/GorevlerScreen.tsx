...
import {
  Card, CardHead, Chip, CONTROL, Empty, Field, PageTitle, Progress,
  RowButton, Secondary, Table, Tabs, TD, TR,
...
export default function GorevlerScreen({
  baslik, alt, ilFiltresi,
}: { baslik: string; alt: string; ilFiltresi?: string }) {
  ...
  return (
    <div>
      ...
      <Card className="mb-[18px] px-[18px] py-4">
        <div className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
          <Field label="Durum" htmlFor="g-durum">
            <select id="g-durum" value={durum} onChange={(e) => setDurum(e.target.value)} className={CONTROL}>
              {durumlar.map((o) => (<option key={o} value={o}>{o}</option>))}
            </select>
          </Field>
          <Field label="Öncelik" htmlFor="g-oncelik">
            <select id="g-oncelik" value={oncelik} onChange={(e) => setOncelik(e.target.value)} className={CONTROL}>
          ...
          <div className="flex items-end">
            <Secondary
              ...
              onClick={() => { setDurum("Tümü"); setOncelik("Tümü"); }}
            >
              Filtreleri temizle
            </Secondary>
          </div>
      ...
      <Card className="mb-[18px]">
        <CardHead title="Görev listesi" hint={`${satirlar.length} görev listeleniyor`} />
        {satirlar.length === 0 ? (
          ...
          <Table
            head={["Görev", "İl / Atölye", "Sorumlu", "Termin", "Durum", "Öncelik", "Gecikme", "İlerleme", "İşlem"]}
            minWidth={1080}
          >
            {satirlar.map((g) => (
              <tr key={g.id} className={TR}>
                <td className={TD}>{g.ad}</td>
                <td className={`${TD} text-[#667085]`}>{g.il} / {g.atolye}</td>
                ...
                <td className={`${TD} ${g.gecikme === "—" ? "text-[#667085]" : "text-[#9B2C2C]"}`}>{g.gecikme}</td>
                <td className={`${TD} min-w-[150px]`}><Progress pct={g.ilerleme} /></td>
                <td className={TD}>
                  <RowButton type="button" onClick={() => setSecili(g)}>Görevi aç</RowButton>
                </td>
              </tr>
            ))}
          </Table>
        ...
        <Card className="p-[18px]">
          <div className="flex flex-wrap items-start gap-3">
            <div className="min-w-[260px]">
              <div className="text-[17px] font-semibold text-[#14213D]">{secili.ad}</div>
              <div className="mt-1 text-sm text-[#3C4657]">{secili.il} / {secili.atolye}</div>
            ...
            {sekme === "Alt adımlar" ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {[
                  ...
                  <div key={k.e} className="rounded-lg border border-[#DDE5F0] px-3.5 py-3">
              ...
              <div className="grid gap-2.5">
                {[
                  { z: secili.guncelleme, a: secili.sorumlu, m: "Görev durumu güncellendi." },
                  { z: "13 Eylül 2026, 09:15", a: "Selin Aksoy (Merkez Operasyon)", m: "Görev oluşturuldu ve il sorumlularına atandı." },
                ].map((h) => (
                  <div key={h.z} className="rounded-lg border border-[#DDE5F0] px-3.5 py-3">
                    <div className="text-[13px] text-[#667085]">{h.z} · {h.a}</div>
